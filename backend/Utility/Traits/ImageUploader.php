<?php
    ...
    trait ImageUploader
    {
        ...
        private $defaultAllowedFileTypes = array("png", "jpeg", "jpg", "gif");
        ...
        public function setAllowedFileTypes(array $setAllowedFileTypes) {
            if (!is_array($setAllowedFileTypes)) {
                throw new MyException("Allowed file types must be an array of the file extensions.");
            }
         ...
         * @throws \App\Http\Exceptions\MyException;
         * @return void
         */
        public function setAllowedFileMaxSize(float $setAllowedFileSize) {
            if (!is_float($setAllowedFileSize)) {
                throw new MyException("Your file size must be a number and less than 100MB");
            }
        ...
        private function checkFile($image_type, $image_size, $canBeNull){
            if (!$canBeNull) {
                if(empty($image_type) ){
                    throw new MyException("No file choosen");
                }
            }
            ...
            if(!in_array($image_type, $allowedImages)){
                throw new MyException("Only ". implode(" | ", $allowedImages)." files are allowed to be uploaded");
            }

            //image size
            if($image_size > $accepted_max_size = ($this->allowedFileSize ?? $this->defaultAllowedFileSize)){
                throw new MyException("All files must be less than ". ($accepted_max_size/1000000). "MB");
            }
        }
